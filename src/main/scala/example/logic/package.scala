...
package object logic {
  ...
  def extractOption[A](
    option: Option[A],
    error: => Error
  ): ZPure[Nothing, State, State, Any, Error, A] =
    option match {
  ...
  def assertThat(
    assertion: Boolean,
    error: => Error
  ): ZPure[Nothing, State, State, Any, Error, Unit] =
    if (assertion) ZPure.unit else ZPure.fail(error)

  def liftEvent[Evt <: Event](
    event: Evt
  )(implicit transition: Transition[Evt]): ZPure[Evt, State, State, Any, Error, Unit] =
    for {
